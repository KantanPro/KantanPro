...
function ktp_add_department_ajax() {
    ...
    if ( ! KTPWP_Department_Manager::table_exists() ) {
        wp_send_json_error( __( '部署テーブルが存在しません。', 'kantanpro' ) );
    }
    ...
    if ( empty( $client_id ) || empty( $contact_person ) || empty( $email ) ) {
        wp_send_json_error( __( '担当者名とメールアドレスを入力してください。', 'kantanpro' ) );
    }
    ...
    if ( $result ) {
        wp_send_json_success( __( '部署を追加しました。', 'kantanpro' ) );
    } else {
        wp_send_json_error( __( '部署の追加に失敗しました。', 'kantanpro' ) );
    }
}
...
function ktp_delete_department_ajax() {
    ...
    if ( empty( $department_id ) ) {
        wp_send_json_error( __( '部署IDが指定されていません。', 'kantanpro' ) );
    }
    ...
    if ( $result ) {
        wp_send_json_success( __( '部署を削除しました。', 'kantanpro' ) );
    } else {
        wp_send_json_error( __( '部署の削除に失敗しました。', 'kantanpro' ) );
    }
}
