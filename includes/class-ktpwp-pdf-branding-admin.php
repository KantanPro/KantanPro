...
final class KTPWP_Pdf_Branding_Admin {
	...
	public static function render_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( '帳票表示設定', 'kantanpro' ); ?></h1>
			<?php if ( isset( $_GET['updated'] ) ) : ?>
				<div class="notice notice-success is-dismissible"><p><?php esc_html_e( '保存しました。', 'kantanpro' ); ?></p></div>
			<?php endif; ?>
			<p><?php esc_html_e( '見積書・案件帳票・一括請求書ごとにレイアウト・表示項目・ロゴ・印影を設定できます。', 'kantanpro' ); ?></p>
			<form method="post" enctype="multipart/form-data">
				...
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="pdf_issuer_name"><?php esc_html_e( '帳票に出す社名', 'kantanpro' ); ?></label></th>
						...
						<p class="description"><?php esc_html_e( '空欄のときはサイト名を使います。', 'kantanpro' ); ?></p></td>
					</tr>
					<tr>
						<th scope="row"><label for="pdf_issuer_address"><?php esc_html_e( '住所・連絡先', 'kantanpro' ); ?></label></th>
						...
						<p class="description"><?php esc_html_e( '空欄のときは一般設定の会社情報を使います。', 'kantanpro' ); ?></p></td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'ロゴ', 'kantanpro' ); ?></th>
						<td>
							...
							<label><input type="checkbox" name="remove_logo" value="1" /> <?php esc_html_e( '削除', 'kantanpro' ); ?></label>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( '印影', 'kantanpro' ); ?></th>
						<td>
							...
							<label><input type="checkbox" name="remove_seal" value="1" /> <?php esc_html_e( '削除', 'kantanpro' ); ?></label>
						</td>
					</tr>
				</table>
				<?php
				foreach ( array(
					KTPWP_Pdf_Document_Kind::ESTIMATE => __( '見積書', 'kantanpro' ),
					KTPWP_Pdf_Document_Kind::ORDER => __( '案件帳票', 'kantanpro' ),
					KTPWP_Pdf_Document_Kind::BULK_INVOICE => __( '一括請求書', 'kantanpro' ),
				) as $kind => $label ) {
	...
	private static function render_kind_fields( $kind, $label, array $values ) {
		...
		<table class="form-table" role="presentation">
			<tr><th><?php esc_html_e( 'レイアウト', 'kantanpro' ); ?></th><td>
				<select name="<?php echo esc_attr( $p ); ?>[layout]">
					<option value="<?php echo esc_attr( KTPWP_Pdf_Document_Settings::LAYOUT_DEFAULT ); ?>" <?php selected( $v( 'layout' ), KTPWP_Pdf_Document_Settings::LAYOUT_DEFAULT ); ?>><?php esc_html_e( '標準', 'kantanpro' ); ?></option>
					<option value="<?php echo esc_attr( KTPWP_Pdf_Document_Settings::LAYOUT_COMPACT ); ?>" <?php selected( $v( 'layout' ), KTPWP_Pdf_Document_Settings::LAYOUT_COMPACT ); ?>><?php esc_html_e( 'コンパクト', 'kantanpro' ); ?></option>
				</select>
			</td></tr>
			<tr><th><?php esc_html_e( 'タイトル（任意）', 'kantanpro' ); ?></th><td><input type="text" name="<?php echo esc_attr( $p ); ?>[title]" value="<?php echo esc_attr( (string) $v( 'title' ) ); ?>" class="regular-text" /></td></tr>
			<tr><th><?php esc_html_e( '挨拶文（任意）', 'kantanpro' ); ?></th><td><textarea name="<?php echo esc_attr( $p ); ?>[lead]" rows="2" class="large-text"><?php echo esc_textarea( (string) $v( 'lead' ) ); ?></textarea></td></tr>
			<tr><th><?php esc_html_e( 'アクセント色', 'kantanpro' ); ?></th><td><input type="text" name="<?php echo esc_attr( $p ); ?>[accent_color]" value="<?php echo esc_attr( (string) $v( 'accent_color' ) ); ?>" pattern="#[0-9A-Fa-f]{6}" /></td></tr>
			<tr><th><?php esc_html_e( '本文サイズ(px)', 'kantanpro' ); ?></th><td><input type="number" min="8" max="16" name="<?php echo esc_attr( $p ); ?>[body_font_size]" value="<?php echo esc_attr( (string) $v( 'body_font_size' ) ); ?>" /></td></tr>
			<tr><th><?php esc_html_e( 'タイトルサイズ(px)', 'kantanpro' ); ?></th><td><input type="number" min="14" max="32" name="<?php echo esc_attr( $p ); ?>[title_font_size]" value="<?php echo esc_attr( (string) $v( 'title_font_size' ) ); ?>" /></td></tr>
			<tr><th><?php esc_html_e( '自社情報の行間', 'kantanpro' ); ?></th><td><input type="number" min="<?php echo (int) KTPWP_Pdf_Document_Settings::ISSUER_LINE_HEIGHT_MIN; ?>" max="<?php echo (int) KTPWP_Pdf_Document_Settings::ISSUER_LINE_HEIGHT_MAX; ?>" step="0.05" name="<?php echo esc_attr( $p ); ?>[issuer_line_height]" value="<?php echo esc_attr( (string) $v( 'issuer_line_height' ) ); ?>" /></td></tr>
			...
			<tr><th><?php esc_html_e( 'ロゴ位置', 'kantanpro' ); ?></th><td>
				<select name="<?php echo esc_attr( $p ); ?>[logo_position]">
					<option value="header" <?php selected( $v( 'logo_position' ), 'header' ); ?>><?php esc_html_e( '上部', 'kantanpro' ); ?></option>
					<option value="footer" <?php selected( $v( 'logo_position' ), 'footer' ); ?>><?php esc_html_e( '下部', 'kantanpro' ); ?></option>
				</select>
			</td></tr>
			<tr><th><?php esc_html_e( 'ロゴ高さ/幅(px)', 'kantanpro' ); ?></th><td>
			...
			<tr><th><?php esc_html_e( '印影サイズ(px)', 'kantanpro' ); ?></th><td>
			...
			<?php self::checkbox_row( $p, 'show_logo', __( 'ロゴ表示', 'kantanpro' ), $v( 'show_logo' ) ); ?>
			<?php self::checkbox_row( $p, 'show_seal', __( '印影表示', 'kantanpro' ), $v( 'show_seal' ) ); ?>
			<?php self::checkbox_row( $p, 'show_tax_column', __( '税率列', 'kantanpro' ), $v( 'show_tax_column' ) ); ?>
			<?php if ( $kind === KTPWP_Pdf_Document_Kind::BULK_INVOICE ) : ?>
			<?php self::checkbox_row( $p, 'show_tax_amount_column', __( '税額列（初期表示）', 'kantanpro' ), $v( 'show_tax_amount_column' ) ); ?>
			<?php endif; ?>
			<?php self::checkbox_row( $p, 'show_remarks_column', __( '備考列', 'kantanpro' ), $v( 'show_remarks_column' ) ); ?>
			<?php self::checkbox_row( $p, 'show_memo', __( 'メモ', 'kantanpro' ), $v( 'show_memo' ) ); ?>
			<?php self::checkbox_row( $p, 'show_output_at', __( '出力日時', 'kantanpro' ), $v( 'show_output_at' ) ); ?>
			<?php self::checkbox_row( $p, 'show_bank_transfer', __( '振込先', 'kantanpro' ), $v( 'show_bank_transfer' ) ); ?>
			<?php self::checkbox_row( $p, 'show_qualified_invoice_number', __( '適格請求書番号', 'kantanpro' ), $v( 'show_qualified_invoice_number' ) ); ?>
			<?php self::checkbox_row( $p, 'logo_keep_aspect_ratio', __( 'ロゴ縦横比固定', 'kantanpro' ), $v( 'logo_keep_aspect_ratio' ) ); ?>
			<?php self::checkbox_row( $p, 'seal_keep_aspect_ratio', __( '印影縦横比固定', 'kantanpro' ), $v( 'seal_keep_aspect_ratio' ) ); ?>
			<?php if ( $kind === KTPWP_Pdf_Document_Kind::BULK_INVOICE ) : ?>
			<tr><th><?php esc_html_e( '余白(mm)', 'kantanpro' ); ?></th><td>
			...
			<tr><th><?php esc_html_e( '封筒窓(mm)', 'kantanpro' ); ?></th><td>
