...
function ktp_handle_sales_ledger_pdf_ajax() {
    ...
    if ( ! current_user_can( 'edit_posts' ) && ! current_user_can( 'ktpwp_access' ) ) {
        wp_send_json_error( __( 'このページにアクセスする権限がありません。', 'kantanpro' ) );
        return;
    }
    ...
    if ( $year < 2000 || $year > date('Y') + 10 ) {
        wp_send_json_error( __( '無効な年度が指定されました。', 'kantanpro' ) );
        return;
    }
    ...
    try {
        ...
        wp_send_json_success( array(
            'pdf_html' => $pdf_html,
            /* translators: 1: 年, 2: 顧客名 */
            'filename' => sprintf( __( '売上台帳_%s年_%s', 'kantanpro' ), $year, date('Ymd') ),
            'year' => $year,
        ...
        wp_send_json_error( __( 'PDF生成中にエラーが発生しました: ', 'kantanpro' ) . $e->getMessage() );
    }
}
...
function ktp_get_sales_ledger_data_for_pdf( $year ) {
    ...
    foreach ( $results as $row ) {
        $sales_data[] = array(
            'id' => $row['id'],
            'date' => wp_date( __( 'Y年m月d日', 'kantanpro' ), strtotime( $row['date'] ) ),
...
function ktp_generate_sales_ledger_pdf_html( $sales_data, $year ) {
    $total_amount = array_sum( array_column( $sales_data, 'total_amount' ) );
    $total_records = count( $sales_data );
    $current_date = wp_date( __( 'Y年m月d日', 'kantanpro' ) );
    ...
    $company_name = trim( $company_name ) !== '' ? $company_name : __( '（自社名未設定）', 'kantanpro' );
    ...
    <div class="sales-ledger-pdf">
        <div class="header" style="text-align: center; margin-bottom: 30px; border-bottom: 2px solid #333; padding-bottom: 20px;">
            ...
            <h1 style="font-size: 24px; margin: 0 0 10px 0; font-weight: bold;">' . esc_html__( '売上台帳', 'kantanpro' ) . '</h1>
            <div style="font-size: 18px; margin-bottom: 10px;">' . esc_html( /* translators: %s: 年度 */ sprintf( __( '%s年度', 'kantanpro' ), $year ) ) . '</div>
            <div style="font-size: 14px; color: #666;">' . esc_html( /* translators: %s: 作成日 */ sprintf( __( '作成日：%s', 'kantanpro' ), $current_date ) ) . '</div>
        </div>

        <div class="summary" style="margin-bottom: 30px; background: #f8f9fa; padding: 20px; border-radius: 8px;">
            <h2 style="font-size: 18px; margin: 0 0 15px 0; color: #333;">' . esc_html__( '年間売上サマリー', 'kantanpro' ) . '</h2>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    <div style="font-size: 14px; color: #666; margin-bottom: 5px;">' . esc_html__( '年間売上合計', 'kantanpro' ) . '</div>
                ...
                <div>
                    <div style="font-size: 14px; color: #666; margin-bottom: 5px;">' . esc_html__( '売上件数', 'kantanpro' ) . '</div>
                    <div style="font-size: 20px; font-weight: bold; color: #4caf50;">' . esc_html( /* translators: %s: 件数 */ sprintf( __( '%s件', 'kantanpro' ), number_format( $total_records ) ) ) . '</div>
                </div>
            </div>
        </div>';
    ...
    if ( ! empty( $monthly_totals ) ) {
        ...
        <div class="monthly-summary" style="margin-bottom: 30px;">
            <h2 style="font-size: 18px; margin: 0 0 15px 0; color: #333;">' . esc_html__( '月別売上サマリー', 'kantanpro' ) . '</h2>
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 12px; border: 2px solid #333;">
                    <thead>
                        <tr style="background: #e3f2fd;">
                            <th style="border: 2px solid #333; padding: 8px; text-align: center; font-weight: bold;">' . esc_html__( '月', 'kantanpro' ) . '</th>
                            <th style="border: 2px solid #333; padding: 8px; text-align: right; font-weight: bold;">' . esc_html__( '売上金額', 'kantanpro' ) . '</th>
                        </tr>
                    </thead>
                    <tbody>';
        for ( $i = 1; $i <= 6; $i++ ) {
            ...
            $html .= '
                        <tr>
                            <td style="border: 2px solid #333; padding: 6px; text-align: center;">' . esc_html( /* translators: %d: 月 */ sprintf( __( '%d月', 'kantanpro' ), $i ) ) . '</td>
    ...
    $html .= '
        <div class="sales-details">
            <h2 style="font-size: 18px; margin: 0 0 15px 0; color: #333;">' . esc_html__( '売上明細', 'kantanpro' ) . '</h2>
            <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-bottom: 20px; border: 2px solid #333;">
                <thead>
                    <tr style="background: #f5f5f5;">
                        ...
                        <th style="border: 2px solid #333; padding: 8px; text-align: center; font-weight: bold;">' . esc_html__( '日付', 'kantanpro' ) . '</th>
                        <th style="border: 2px solid #333; padding: 8px; text-align: center; font-weight: bold;">' . esc_html__( '顧客名', 'kantanpro' ) . '</th>
                        <th style="border: 2px solid #333; padding: 8px; text-align: center; font-weight: bold;">' . esc_html__( '案件名', 'kantanpro' ) . '</th>
                        <th style="border: 2px solid #333; padding: 8px; text-align: center; font-weight: bold;">' . esc_html__( '商品・サービス', 'kantanpro' ) . '</th>
                        <th style="border: 2px solid #333; padding: 8px; text-align: center; font-weight: bold;">' . esc_html__( '売上金額', 'kantanpro' ) . '</th>
                    </tr>
                </thead>
                <tbody>';
        ...
        $html .= '
                    <tr>
                        <td colspan="6" style="border: 2px solid #333; padding: 20px; text-align: center; color: #666;">
                            ' . esc_html__( '対象年度の売上データがありません。', 'kantanpro' ) . '
                        </td>
                    </tr>';
    }
    ...
    $html .= '
                ...
                <tfoot>
                    <tr style="background: #f0f8ff; font-weight: bold;">
                        <td colspan="5" style="border: 2px solid #333; padding: 8px; text-align: right;">' . esc_html__( '合計', 'kantanpro' ) . '</td>
