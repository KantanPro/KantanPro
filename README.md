# KantanPro (KTPWP) - Version 1.2.87
...
### Version 1.2.87 - 2026年5月24日

- ヘッダー更新バッジ：`ktpwp_update_badge_available` を `wp_add_inline_script` で渡すよう変更し、フロント側で真偽値が正しく評価されるよう修正

### Version 1.2.86 - 2026年5月20日

- 定期更新チェックを追加し、管理画面未訪問でもフロント側で更新状態の維持・反映ができるよう改善
- ヘッダー更新バッジの軽量ポーリングを追加し、ページ表示時に更新有無を再判定してバッジ表示を自動同期

### Version 1.2.85 - 2026年5月20日

- ヘッダーの更新チェックボタンに、更新あり時の赤丸バッジ表示を追加（「後で」で吹き出しを閉じてもバッジは残る）
- 更新チェッカーに `has_header_update_badge()` を追加し、通知設定・権限・保存済み更新情報から表示可否を判定

### Version 1.2.84 - 2026年5月20日

- 一括請求プレビューの明細テーブルヘッダーにクラスを追加し、背景色・下線色を KantanBiz 相当のピンク系に調整

### Version 1.2.83 - 2026年5月20日

- 一括請求の月別グループで、締日表示を顧客マスタの締め日と対象年月から算出するよう変更（完了日ベースを廃止。「末」「末日」対応）

### Version 1.2.82 - 2026年5月20日

- `v1.2.81` 以降のコミット・ファイル差分なし（バージョン番号とリリースノートのみ更新のメンテナンスリリース）

### Version 1.2.81 - 2026年5月8日

- WordPress標準更新で GitHub `zipball_url` 由来の展開フォルダを `KantanPro` に正規化し、更新後にプラグインが無効化されるリスクを修正
